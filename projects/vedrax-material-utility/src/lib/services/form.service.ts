...
import { DescriptorFormControl, DescriptorProperty, MatrixColumn, NVP } from '../entities';
...
export class FormService {
  ...
  createFormControl(descriptor: DescriptorFormControl): FormControl | FormArray {
    ...
    if (!descriptor) {
      throw new Error('A form control descriptor is mandatory');
    }
  ...
  /**
   * Helper method for creating a {@link FormControl} with value.
   * 
   * @param descriptor 
   * @param value 
   * @returns 
  ...
  /**
   * Helper method for creating autocomplete with value
   * @param descriptor 
   * @returns 
   */
  ...
  getComponent(control: AbstractControl, controlName: string): FormControl {
    return control.get(controlName) as FormControl || new FormControl();
  }

  getComponents(control: AbstractControl, controlName: string): FormArray {
    return control.get(controlName) as FormArray || new FormArray([]);
  }
}
